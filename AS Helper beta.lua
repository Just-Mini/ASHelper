...
local rule_align					= new.int(AshSettings.MainSettings.rule_align)
...
local windows = {
	...
	imgui_changelog					= new.bool(AshSettings.MainSettings.changelog),
	imgui_first_launch				= {},
...
local sobes_settings = {
	pass							= new.bool(AshSettings.MainSettings.Sobes.pass),
	medcard							= new.bool(AshSettings.MainSettings.Sobes.medcard),
	wbook							= new.bool(AshSettings.MainSettings.Sobes.wbook),
	licenses						= new.bool(AshSettings.MainSettings.Sobes.licenses),
}
local tagbuttons = {
...
local imgui_fm = imgui.OnFrame(
	...
	function(player)
		...
		imgui.Begin(u8'Меню взаимодействия', windows.imgui_fm, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar)
			...
			imgui.BeginChild('##fmmainwindow', imgui.ImVec2(300, -1), false)
						...
						if #sellList > 0 and sellList.sellPerson == fastmenuID then
							...
							if sellList.sellLicense ~= 0 then
								if sellList.checking_medcard.status ~= 0 then
									...
									if status > 3 and status < 11 then
										local statusText = {
											[4] = 'Клиент отошел слишком\nдалеко',
											[5] = 'У клиента уже есть эта лицензия сроком 3+ дней',
											[6] = 'У клиента недостаточно денег для покупки',
										...
										local statusReplies = {
											...
											[10] = 'К сожалению ваша мед. карта не в норме.',
										}
										local popup = function()
				...
				elseif newwindowtype[0] == 2 then
					...
					if sobesetap[0] == 0 then
						...
						if imgui.Button(u8'Поприветствовать', imgui.ImVec2(285,30)) then
							sendchatarray(AshSettings.MainSettings.playcd, {
								{'Здравствуйте, я %s %s, Вы пришли на собеседование?', AshSettings.ScannedVariables.RankNames[AshSettings.MainSettings.myrankint], AshSettings.MainSettings.replaceash and AshSettings.MainSettings.replaceashto or 'Автошколы'},
								{'/do На груди висит бейджик с надписью %s %s.', AshSettings.ScannedVariables.RankNames[AshSettings.MainSettings.myrankint], #AshSettings.MainSettings.myname < 1 and gsub(sampGetPlayerNickname(select(2,sampGetPlayerIdByCharHandle(playerPed))), '_', ' ') or u8:decode(AshSettings.MainSettings.myname)},
							})
						end
						imgui.SetCursorPosX(7.5)
						...
						if imgui.IsItemHovered() then
							...
							if imgui.IsMouseReleased(0) then
								if not inprocess then
									local s = AshSettings.MainSettings.Sobes
						...
						if imgui.BeginPopup('##redactdocuments') then
							if imgui.ToggleButton(u8'Проверять паспорт', sobes_settings.pass) then
								AshSettings.MainSettings.Sobes.pass = sobes_settings.pass[0]
								inicfg.save(AS_Settings,'AS Helper')
							end
							if imgui.ToggleButton(u8'Проверять мед. карту', sobes_settings.medcard) then
								AshSettings.MainSettings.Sobes.medcard = sobes_settings.medcard[0]
								inicfg.save(AS_Settings,'AS Helper')
							end
							if imgui.ToggleButton(u8'Проверять трудовую книгу', sobes_settings.wbook) then
								AshSettings.MainSettings.Sobes.wbook = sobes_settings.wbook[0]
								inicfg.save(AS_Settings,'AS Helper')
							end
							if imgui.ToggleButton(u8'Проверять лицензии', sobes_settings.licenses) then
								AshSettings.MainSettings.Sobes.licenses = sobes_settings.licenses[0]
								inicfg.save(AS_Settings,'AS Helper')
							end
							imgui.EndPopup()
						end
				...
				elseif newwindowtype[0] == 3 then
					...
					elseif leadertype[0] == 2 then
						...
						imgui.PushStyleVarVec2(imgui.StyleVar.WindowPadding,imgui.ImVec2(10,10))
						imgui.Combo('##chooserank9', Ranks_select, new['const char*'][9]({u8('[1] '..AshSettings.ScannedVariables.RankNames[1]), u8('[2] '..AshSettings.ScannedVariables.RankNames[2]),u8('[3] '..AshSettings.ScannedVariables.RankNames[3]),u8('[4] '..AshSettings.ScannedVariables.RankNames[4]),u8('[5] '..AshSettings.ScannedVariables.RankNames[5]),u8('[6] '..AshSettings.ScannedVariables.RankNames[6]),u8('[7] '..AshSettings.ScannedVariables.RankNames[7]),u8('[8] '..AshSettings.ScannedVariables.RankNames[8]),u8('[9] '..AshSettings.ScannedVariables.RankNames[9])}), 9)
						imgui.PopStyleVar()
						imgui.PopItemWidth()
						imgui.SetCursorPosX(7.5)
							...
							imgui.BeginGroup()
								...
								imgui.Columns(6)
									for i = 1, #AshSettings.ScannedVariables.PriceList do
										...
										for i = 1, 3 do
											if prices[i] ~= nil then imgui.Text(string.separate(tostring(prices[i]))..'$') else imgui.Text('-') end
											imgui.NextColumn()
										end
			...
			for k, v in pairs(windows.imgui_first_launch) do
				if v == 1 then
					...
					if imgui.Button(u8'Перенести настройки', imgui.ImVec2(179, 35)) then
						...
						for k, v in pairs(AshSettings.MainSettings.Sobes) do
							if configuration.sobes_settings[k] ~= nil then
								AshSettings.MainSettings.Sobes[k] = configuration.sobes_settings[k]
							end
						end
...
local imgui_tasks = imgui.OnFrame(
	function() return AshSettings.TaskChecker.state end,
	function(player)
			...
			if notify.msg[k] and notify.msg[k].active then
				...
				if notify.pos.y - i * 21 > 0 then
					...
					imgui.Begin(u8('Notify ##'..k), _, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoScrollbar)
						...
						DrawList:PathStroke(AshSettings.MainSettings.ASChatColor.color, false, 3)
						...
						imgui.PushFont(font[16])
						imgui.SetCursorPos(imgui.ImVec2(59, 10))
						imgui.TextColoredRGB('{MC}autoschool/notify')
						imgui.PopFont()
	...
	if id == 220 then
		...
		if (raknetBitStreamReadInt8(bs) == 17) then
			...
			if str:find('event.employment.updateData') and AshSettings.TaskChecker.state == true then
				...
				local tasks = {
					[316] = {
						description = 'Для завершения задания необходимо переодеться в рабочую форму и провести на службе нескольких часа онлайн (без АФК).',
						text = 'Провести на службе несколько часов'
					},
					[317] = {
						description = 'Для завершения задания необходимо подойти к компьютеру в интерьере вашей организации и начать сдачу фракционного теста. Местоположение свободного компьютера отмечено на миникарте.',
						text = 'Пройти тест на компьютере в организации'
					},
					[318] = {
						description = 'Для завершения задания необходимо заступить на любой свободный пост в интерьере фракции и простоять на нем нескольких минут (без АФК).',
						text = 'Простоять на посту несколько минут'
					},
					[319] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензий на авто нуждающимся игрокам (/givelicense).',
						text = 'Продать лицензии на авто'
					},
					[320] = {
						description = 'Для завершения задания необходимо написать в РП чат организации (/r) нескольких разных сообщений.',
						text = 'Написать несколько сообщение в чат организации'
					},
					[321] = {
						description = 'Для завершения задания необходимо принять участие в Role Play мероприятии в вашей организации и получить подтверждение об участии. Выдать подтверждение об участии в RP мероприятии может руководство фракции через команду /fractionrp.',
						text = 'Принять участие в Role-Play мероприятии'
					},
					[322] = {
						description = 'Для завершения задания необходимо отстоять испытательный срок в нескольких часов на вашей должности. Обратите внимание! Время засчитывается только когда вы находитесь онлайн и во фракционной форме.',
						text = 'Отстоять испытательный срок на должности'
					},
					[323] = {
						description = 'Для завершения задания необходимо подойти к компьютеру в интерьере вашей организации и начать сдачу фракционного теста. Местоположение свободного компьютера отмечено на миникарте.',
						text = 'Пройти тест на компьютере в организации'
					...
					[326] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на мото-транспорт нуждающимся игрокам (/givelicense).',
						text = 'Продать лицензии на мото'
					},
					[334] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на рыбалку нуждающимся игрокам (/givelicense).',
						text = 'Продать лицензии на рыбалку'
					},
					[343] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензий на вождение водного транспорта нуждающимся игрокам (/givelicense).',
						text = 'Продать лицензии на плавание'
					},
					[347] = {
						description = 'Для завершения задания необходимо заступить на любой свободный пост в интерьере фракции и простоять на нем нескольких минут (без АФК)',
						text = 'Простоять на посту несколько минут'
					...
					[348] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на авто нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на авто'
					},
					[349] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на мото-транспорт нуждающимся игрокам (/givelicense)',
					...
					[351] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на вождение водного транспорта нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на плавание'
					},
					[352] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на оружие нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на оружие'
					},
					[353] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на охоту нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на охоту'
					},
					[354] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на раскопки нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на раскопки'
					},
					[355] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на работу механиком нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на работу механиком'
					},
					[356] = {
						description = 'Для завершения задания необходимо выдать нескольких лицензии на полеты нуждающимся игрокам (/givelicense)',
						text = 'Продать лицензии на полёты'
					},
					[357] = {
						description = 'Для завершения задания необходимо написать в РП чат организации (/r) нескольких разных сообщений',
						text = 'Написать несколько сообщение в чат организации'
					...
					[358] = {
						description = 'Для завершения задания необходимо написать в общий гос. чат (/d) нескольких разных сообщений',
						text =  'Написать несколько сообщение в чат департамента'
					},
					[359] = {
						description = 'Для завершения задания необходимо принять участие в Role Play мероприятии в вашей организации и получить подтверждение об участии. Выдать подтверждение об участии в RP мероприятии может руководство фракции через команду /fractionrp.',
						text = 'Принять участие в Role-Play мероприятии'
					...
					[632] = {
						description = 'Для завершения задания необходимо установить или отремонтировать нескольких дорожных знаков.',
						text = 'Установить несколько дорожных знаков'
					},
				}
		...
		if find(text, 'Центр лицензирования') then
			for DialogLine in gmatch(text, '[^\r\n]+') do
				...
				if tonumber(getStatsRank) then
					...
					local rang = tostring(nameRankStats):gsub(' ','')
	...
	if dialogId == 2015 then 
		for line in gmatch(text, '[^\r\n]+') do
			...
			if name and rank then
				name, rank = tostring(name):gsub(' ',''), tonumber(rank)
				...
				if find(cmd, '/'..AshSettings.MainSettings.usefastmenucmd..' %d+') then
					...
					if sampIsPlayerConnected(param) then
						if doesCharExist(select(2,sampGetCharHandleBySampPlayerId(param))) then
							...
							ASHelperMessage('Зажмите {MC}ПКМ{WC} для того, чтобы скрыть курсор. {MC}ESC{WC} для того, чтобы закрыть меню.')
							windows.imgui_fm[0] = true
						else
...
function onScriptTerminate(script, quitGame)
	if script == thisScript() then
		...
		sampShowDialog(536472, '{ff6633}autoschool/helper{ffffff} Скрипт был выгружен сам по себе.', [[
			...
			if AshSettings.Checker.confirm then
				...
				for k, member in ipairs(ch.online) do
					if k <= tonumber(checker_variables.online.online) then
						...
						local mute = cfgch.show_demorgan and member.demorgan and ' || Demorgan' or ''
						...
						local render_text = format('%s%s%s%s%s%s%s%s%s', rank, nick, id, afk, warns, specwarns, quests, mute, near, note)
						local text_lenght = renderGetFontDrawTextLength(ch.font, render_text)
