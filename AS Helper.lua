...
function main()
	...
	ASHelperMessage("Введите /"..cmdcmds.." чтобы узнать все команды.")
	imgui.Process = false
	...
	sampRegisterChatCommand('skip', function()
		if skiporcancel == false then
			...
			cansell = true
			inprocess = false
			ASHelperMessage('Вы пропустили ожидание мед.карты')
			selllic(tempid..' оружие')
		else
			ASHelperMessage('Вам нечего пропускать!')
		end
	end)
	sampRegisterChatCommand('cancelmc', function()
		if skiporcancel == false then
			...
			cansell = false
			inprocess = false
			ASHelperMessage('Вы отменили ожидание мед.карты')
		else
			ASHelperMessage('Вам нечего отменять!')
		end
	end)
					...
					if not sampIsChatInputActive() then
						...
						if targettingid ~= -1 then
							if not imgui_fm.v then
								...
								ASHelperMessage("Вы использовали меню быстрого доступа на: "..sampGetPlayerNickname(fastmenuID).."["..fastmenuID.."]")
								windowtype.v = 0
								imgui_fm.v = true
							end
						end
					...
					elseif lictype == 'оружие' then
						if cansell == false then
							...
							if sampIsPlayerConnected(sellto) or sellto == myid then
								...
								ASHelperMessage('Началось ожидание показа мед.карты, чтобы отменить его введите /cancelmc, чтобы пропустить введите /skip')
								skiporcancel = false
...
function sobesaccept2()
	lua_thread.create(function()
		local rangint = configuration.main_settings.myrankint
		if inprocess == true then
			ASHelperMessage("Не торопитесь, вы уже отыгрываете что-то!")
		else
		inprocess = true
...
if sampevcheck then
			...
			if text:find('Срок действия') then
				...
				elseif skiporcancel == false then
					if text:find("Имя: "..choosedname) then
						...
						ASHelperMessage('Вам подкинули мед.карту, ожидание началось заново! Чтобы отменить его введите /cancelmc')
	...
	function sampev.onServerMessage(color, message)
		...
		if message:find("%[Информация%] {FFFFFF}(.+) принял ваше предложение вступить к вам в организацию.") and waitingaccept then
			if message:match("%[Информация%] {FFFFFF}(.+)") == sampGetPlayerNickname(waitingaccept) then
				sampSendChat("/giverank "..waitingaccept.." 2")
				waitingaccept = false
				ASHelperMessage(sampGetPlayerNickname(waitingaccept).." принял ваше предложение вступить в Автошколу и был повышен до должности Консультанта.")
				return false
			end
		end
	end
...
function onWindowMessage(msg, wparam, lparam)
    if wparam == 0x1B then
    	if imgui_settings.v or imgui_fm.v or imgui_cmds.v or imgui_stats.v or imgui_binder.v then
	...
	function imgui.OnDrawFrame()
		...
		if imgui_settings.v == true then
			...
			if scriptinfo then
				...
				imgui.Text(u8'Версия 1.8',imgui.ImVec2(70,20))
				...
				imgui.Text(u8
[[
Добавлена функция не отыгрывать команды;
Добавлена функция принятия на 2-ой ранг;
Изменены некоторые системы;
Изменён цвет кнопок;
Немного изменён биндер;
При зажатом ALT теперь пропадает курсор.
]]
				,imgui.ImVec2(70,20))
			end
			imgui.End()
		...
		elseif imgui_fm.v == true then
			imgui.SetNextWindowSize(imgui.ImVec2(300, 517), imgui.Cond.FirstUseEver)
			imgui.SetNextWindowPos(imgui.ImVec2(ScreenX / 2 , ScreenY / 2),imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(u8"Меню быстрого доступа", imgui_fm, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoCollapse)
			...
			elseif windowtype.v == 1 then -- ПРОДАТЬ ЛИЦ  ПРОДАТЬ ЛИЦ  ПРОДАТЬ ЛИЦ  ПРОДАТЬ ЛИЦ  ПРОДАТЬ ЛИЦ  ПРОДАТЬ ЛИЦ  
				...
				if imgui.Button(u8'Продать лицензию на '..u8(whichlic), imgui.ImVec2(285,30)) then
					if not inprocess then
						ComboBox_select.v = 0
						selllic(tostring(fastmenuID).." "..tostring(whichlic))
						disableallimgui()
					else
						ASHelperMessage("Не торопитесь, вы уже отыгрываете что-то!")
					end
			...
			elseif windowtype.v == 4 then -- ДАТЬ РАНГ  ДАТЬ РАНГ  ДАТЬ РАНГ  ДАТЬ РАНГ  ДАТЬ РАНГ  ДАТЬ РАНГ  
				...
				if imgui.Button(u8'Повысить сотрудника '..fa.ICON_FA_ARROW_UP, imgui.ImVec2(270,30)) then
		...
		if imgui_binder.v == true then
			...
			imgui.SetNextWindowPos(imgui.ImVec2(ScreenX / 2 , ScreenY / 2),imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(u8"Настройки биндера", imgui_binder, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoCollapse)
		...
		if imgui_cmds.v == true then
			...
			imgui.Begin(u8"Команды хелпера", imgui_cmds, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoBringToFrontOnFocus)
