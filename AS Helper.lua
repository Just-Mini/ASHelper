...
function imgui.OnDrawFrame()
	if imgui_settings.v == true then
		...
		if scriptinfo then
			...
			imgui.Separator()
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize(u8'Исправлено множество багов;').x) / 2)
			imgui.Text(u8'Исправлено множество багов;',imgui.ImVec2(70,20))
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize(u8'Добавлен биндер;').x) / 2)
			imgui.Text(u8'Добавлен биндер;',imgui.ImVec2(70,20))
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize(u8'Кардинально изменено меню настроек;').x) / 2)
			imgui.Text(u8'Кардинально изменено меню настроек;',imgui.ImVec2(70,20))
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize(u8'Добавлены новые баги').x) / 2)
			imgui.Text(u8'Добавлены новые баги',imgui.ImVec2(70,20))
		end
		imgui.End()
	...
	elseif imgui_sobes.v == true then
		...
		elseif sobesetap == 5 then
			lastsobesetap = sobesetap
			imgui.SetCursorPosX((imgui.GetWindowWidth() - 285) / 2)
			if imgui.Button(u8"Работа ли вы уже в организациях ЦА? Если да, то расскажите подробнее", imgui.ImVec2(285,30)) then
				if not inprocess then
					sobes5()
					sobesetap = sobesetap + 1
				else
...
function sobesaccept1()
	lua_thread.create(function()
		...
		inprocess = true
		sampSendChat("/me взяв документы из рук человека напротив начал их проверять")
		wait(cd)
...
function checkbibl()
	if not inicfgcheck then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Отсутствует библиотека inicfg. Скрипт был отключен во избежание крашей.", 0xff6633)
		thisScript():unload()
	end
	if not vkeyscheck then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Отсутствует библиотека vkeys. Скрипт был отключен во избежание крашей.", 0xff6633)
		thisScript():unload()
	end
	if not imguicheck then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Отсутствует библиотека imgui. Скрипт был отключен во избежание крашей.", 0xff6633)
		thisScript():unload()
	end
	if not sampevcheck then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Отсутствует библиотека samp events. Скрипт был отключен во избежание крашей.", 0xff6633)
		thisScript():unload()
	end
	if not encodingcheck then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Отсутствует библиотека encoding. Скрипт был отключен во избежание крашей.", 0xff6633)
		thisScript():unload()
	end
	if not facheck then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Отсутствует библиотека fAwesome5. Скрипт был отключен во избежание крашей.", 0xff6633)
		thisScript():unload()
	end
	if not doesFileExist('moonloader/resource/fonts/fa-solid-900.ttf') then
		sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Не найден файл шрифта, пытаюсь его установить.", 0xff6633)
		...
		downloadUrlToFile('https://github.com/Just-Mini/biblioteki/raw/main/fa-solid-900.ttf', 'moonloader/resource/fonts/fa-solid-900.ttf', function(id, status)
			if status == dlstatus.STATUSEX_ENDDOWNLOAD then
				if doesFileExist('moonloader/resource/fonts/fa-solid-900.ttf') then
					sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Файл шрифта успешно установлен.", 0xff6633)
				else
					sampAddChatMessage("{ff6633}[ASHelper] {EBEBEB}Файл шрифта не удалось установить, обратитесь к разработчику за помощью. JustMini#6291", 0xff6633)
					thisScript():unload()
				end
			end
		end)
	end
	if not doesFileExist('moonloader/config/updateashelper.ini') then
